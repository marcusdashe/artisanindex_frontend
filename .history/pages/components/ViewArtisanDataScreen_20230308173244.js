...
function ViewArtisanDataScreen() {
  ...
  function handleSearchInputChange(event) {
    ...
    switch (selectedValue) {
      ...
      case "phone":
        setArtisans(
          artisans.filter((artisan) => artisan.phone === searchInput)
        );
      case "state":
        setArtisans(
          artisans.filter((artisan) => artisan.state === searchInput)
        );
        break;
      case "gender":
        setArtisans(
          artisans.filter((artisan) => artisan.gender === searchInput)
        );
      case "fullname":
        setArtisans(
          artisans.filter((artisan) => artisan.fullname === searchInput)
        );
      case "city":
        setArtisans(artisans.filter((artisan) => artisan.city === searchInput));
    }
  }
