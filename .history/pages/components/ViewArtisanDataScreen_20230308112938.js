...
function ViewArtisanDataScreen() {
  ...
  function fetchData() {
    axios
      .get(
        `https://example.com/api/artisan-data?search=${searchInput}&filter=${selectedValue}`
      )
      .then((response) => {
    ...
    <div className="w-full h-[100vh]">
      ...
      <div className="w-full h-[90vh] p-4 overflow-y-scroll overflow-x-hidden">
        ...
        <select
          ...
          className="block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#8D161A] focus:border-[#8D161A] sm:text-sm"
        >
        ...
        <button
          className="bg-[#8D161A] text-white rounded-md py-2 px-4  hover:hover:bg-[#4a3e3e] duration-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
          onClick={fetchData}
        >
          Fetch Data
        </button>
