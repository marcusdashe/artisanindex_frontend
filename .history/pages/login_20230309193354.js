...
export default function Login() {
  ...
  const loginUrl = `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}api/auth/login`;
  ...
  const handleSubmit = async (e) => {
    ...
    if (response && response.data.status) {
      router.replace(`/${response.data.jwt}/dashboard`);
    }
  };
    ...
    <div className="flex flex-col items-center justify-center min-h-screen py-2">
      ...
      <div className="flex flex-col items-center justify-center bg-white shadow-md rounded-lg p-8">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col w-full">
          ...
          <button
            type="submit"
            className="bg-[#8D161A] text-white rounded-md py-2 px-4 font-medium hover:bg-[#4a3e3e] duration-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            Login
          </button>
        </form>
